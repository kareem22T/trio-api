...
<div class="card p-3 mb-3" id="services_wrapper">
    <div class="card-header mb-3">
        <h3 class="text-danger text-center mb-0">Are you sure you want to delete this service?</h3>
    </div>
    ...
    <div class="form-group w-100 d-flex justify-content-center" style="gap: 16px">
        <a href="{{ route("admin.services.show") }}" class="btn btn-secondary w-25">Cancel</a>
...
createApp({
    ...
    methods: {
        handleChangeThumbnail(service) {
            this.thumbnail = service.target.files[0]
            this.thumbnail_path = URL.createObjectURL(service.target.files[0])
        },
        async deleteCat() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`{{ route("admin.services.delete") }}`, {
                    id: this.id
                },
                );
                if (response.data.status === true) {
                    ...
                    setTimeout(() => {
                        $('.loader').fadeOut()
                        $('#errors').fadeOut('slow')
                        window.service.href = '{{ route("admin.services.show") }}'
                    }, 1300);
                } else {
                    $('.loader').fadeOut()
                    document.getElementById('errors').innerHTML = ''
